// pages/reports.js — reports page with filters and CSV export
...
initPage("reports", "Reports", "Generate and export attendance reports");
...
var sel = c.querySelector("#rep-student");
...
var rc = c.querySelector("#rep-results"), lR = [];

function runReport() {
  var sid = sel.value, from = c.querySelector("#rep-from").value, to = c.querySelector("#rep-to").value;
  ...
  rc.querySelectorAll("[data-href]").forEach(function (r) { r.addEventListener("click", function () { window.location.href = r.getAttribute("data-href"); }); });
}
...
c.querySelector("#rep-run").addEventListener("click", runReport);
c.querySelector("#rep-csv").addEventListener("click", exportRep);
c.querySelector("#rep-print").addEventListener("click", function () { window.print(); });
