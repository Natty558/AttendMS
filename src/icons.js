// icons.js — SVG icon library and logo
...
export function classIconFor(name) {
...
export function icon(n, c) {
  return '<span class="icon' + (c ? " " + c : "") + '">' + (SVG[n] || "") + "</span>";
}

export function navIcon(n) {
  return '<span class="nav-icon">' + (SVG[n] || "") + "</span>";
}
