/*
 * Student Attendance Management System v4.0
 * Pure HTML, CSS, JavaScript. localStorage-based auth (register/login/forgot password).
 * Professional SVG logo. SVG icons on class cards. Real-world photos from Pexels for login bg and empty states only.
 * Default avatars are SVG icon placeholders (not photos). Uploaded photos override the icon.
 * All data stored in localStorage.
 */
(function () {
  "use strict";

  // Professional logo - self-contained SVG with gradient background
  var LOGO_SVG =
  ...
  // Pexels stock photos — login background + empty states only
  var IMG = {
  ...
  var state = {
    ...
    attendanceDraft: {}, attendanceNotes: {}, authLoading: false
  };
  ...
  function toast(msg) {
    ...
    setTimeout(function () { el.style.opacity = "0"; el.style.transition = "opacity 0.3s"; setTimeout(function () { el.remove(); }, 300); }, 2400);
  }
  function persist() {
    save(LS.students, state.students); save(LS.attendance, state.attendance);
    save(LS.session, state.session); save(LS.lecturer, state.lecturer); save(LS.classes, state.classes);
    save(LS.accounts, state.accounts); save(LS.currentAccount, state.currentAccount);
  }
  function applyTheme() { document.documentElement.setAttribute("data-theme", state.theme); }
  function toggleTheme() {
    state.theme = state.theme === "dark" ? "light" : "dark";
    save(LS.theme, state.theme); applyTheme();
    var b = document.getElementById("theme-toggle-btn");
    if (b) b.innerHTML = state.theme === "dark" ? SVG.sun : SVG.moon;
  }

  // Default avatar is now an SVG icon placeholder — NOT a photo.
  // Only uploaded photos show as actual images.
  function avatarPlaceholderHTML(sizeClass) {
    var cls = sizeClass ? " " + sizeClass : "";
    return '<span class="avatar-placeholder-icon' + cls + '">' + SVG.userPlaceholder + '</span>';
  }
  ...
  function passwordStrength(pw) {
    ...
    if (score <= 3) return { level: "medium", label: "Medium", bars: 2 };
  ...
  function registerAccount(name, email, password) {
    ...
    var existing = state.accounts.find(function (a) { return a.email.toLowerCase() === email.toLowerCase(); });
    if (existing) return "An account with this email already exists. Please sign in instead.";
    var account = {
    ...
    state.lecturer = { name: account.name, email: account.email, phone: account.phone, photo: account.photo };
    state.session = { email: account.email, loginAt: Date.now() };
    persist();
    return null;
  }
  ...
  function loginAccount(email, password) {
    ...
    if (!validateEmail(email)) return "Please enter a valid email address.";
    var account = state.accounts.find(function (a) { return a.email.toLowerCase() === email.toLowerCase(); });
    if (!account) return "No account found with this email. Please register first.";
    if (account.passwordHash !== hashPassword(password)) return "Incorrect password. Please try again.";
    state.currentAccount = account;
    state.lecturer = { name: account.name, email: account.email, phone: account.phone || "", photo: account.photo || "" };
    state.session = { email: account.email, loginAt: Date.now() };
    persist();
  ...
  function resetPassword(email, newPassword) {
    ...
    var account = state.accounts.find(function (a) { return a.email.toLowerCase() === email.toLowerCase(); });
    if (!account) return "No account found with this email address.";
  ...
  function logout() {
    state.session = null; state.currentAccount = null;
    persist(); state.view = "dashboard"; state.authView = "login";
    render();
  }
  ...
  // ===== Class CRUD =====
  function addClass(d) {
    var c = { id: uid("CLS-"), name: d.name.trim(), code: d.code.trim(), description: (d.description || "").trim(), createdAt: new Date().toISOString() };
    state.classes.push(c); persist(); return c;
  }
  function updateClass(id, d) {
    var c = findClass(id); if (!c) return;
    c.name = d.name.trim(); c.code = d.code.trim(); c.description = (d.description || "").trim();
    persist();
  }
  ...
  function saveAttendance(date, records, notes) {
    state.attendance = state.attendance.filter(function (a) { return a.date !== date; });
    records.forEach(function (r) {
      state.attendance.push({ id: uid("ATT-"), studentId: r.studentId, date: date, status: r.status, note: notes && notes[r.studentId] ? notes[r.studentId].trim() : "", recordedAt: new Date().toISOString() });
    });
    persist();
  }
  function studentRecords(sid) { return state.attendance.filter(function (a) { return a.studentId === sid; }); }
  function attendanceSummary(sid) {
  ...
  function atRiskStudents() {
    return state.students.filter(function (s) { var sm = attendanceSummary(s.id); return sm.rate < 75 && sm.total > 0; });
  }
  function attendanceStreak(sid) {
    var r = studentRecords(sid).sort(function (a, b) { return b.date.localeCompare(a.date); });
    var st = 0;
    for (var i = 0; i < r.length; i++) { if (r[i].status === "present" || r[i].status === "late") st++; else break; }
    return st;
  }
  function dailyRate(date) {
    var r = getAttendanceForDate(date); if (!r.length) return null;
    var p = r.filter(function (a) { return a.status === "present" || a.status === "late"; }).length;
    return pct(p, r.length);
  }

  // ===== Export/Import =====
  function exportCSV(fn, rows) {
  ...
  function exportData() {
    var d = { students: state.students, attendance: state.attendance, classes: state.classes, lecturer: state.lecturer, accounts: state.accounts, exportedAt: new Date().toISOString() };
    var b = new Blob([JSON.stringify(d, null, 2)], { type: "application/json" });
    var u = URL.createObjectURL(b); var a = document.createElement("a");
    a.href = u; a.download = "attendance-backup-" + todayISO() + ".json";
  ...
  function dSeg(cx, cy, r, C, len, off, c) {
    if (len <= 0) return "";
    return '<circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="none" stroke="' + c + '" stroke-width="22" stroke-dasharray="' + len + " " + (C - len) + '" stroke-dashoffset="' + off + '" transform="rotate(-90 ' + cx + " " + cy + ')" style="transition:stroke-dasharray 0.6s ease,stroke-dashoffset 0.6s ease"/>';
  }
  function barChart(labels, values, mx) {
    if (!values.length || !mx) return '<div class="empty-state" style="padding:2rem;"><p>No attendance data yet</p></div>';
    var cH = 200, pad = 40, bW = 24, gap = 16, tW = labels.length * (bW + gap) + pad * 2, sH = cH - pad * 2;
    var bars = "", grid = "";
    for (var g = 0; g <= 4; g++) { var gy = pad + (sH / 4) * g; grid += '<line x1="' + pad + '" y1="' + gy + '" x2="' + (tW - pad) + '" y2="' + gy + '" stroke="var(--border)" stroke-width="1"/>'; }
  ...
  function trendChart() {
    ...
    for (var i = days - 1; i >= 0; i--) {
      ...
      lb.push(fmtDateShort(iso)); vl.push(p); if (p > mx) mx = p;
    }
  ...
  function render() {
    app.innerHTML = "";
    if (!isLoggedIn()) { app.appendChild(renderAuth()); return; }
  ...
  function renderAuth() {
    ...
    var v = state.authView;
    if (v === "register") d.appendChild(renderRegister());
    else if (v === "forgot") d.appendChild(renderForgot());
  ...
  function renderLogin() {
    ...
    d.innerHTML =
      ...
      '<h1>Welcome Back</h1>' +
      '<p class="subtitle">Sign in to your lecturer account</p>' +
      ...
      '<div class="password-field"><input type="password" id="password" placeholder="Enter your password" required autocomplete="current-password" />' +
      '<button type="button" class="password-toggle" id="pw-toggle">' + SVG.eye + '</button></div></div>' +
      '<button type="submit" class="btn btn-primary" id="login-btn">Sign In</button>' +
      '</form>' +
      '<div class="auth-links">' +
      '<div class="auth-divider">or</div>' +
      '<span class="auth-link">Don\'t have an account? <a id="go-register">Create one</a></span>' +
      '<span class="auth-link"><a id="go-forgot">Forgot your password?</a></span>' +
      '</div>';
    var pwInput = d.querySelector("#password");
    var pwToggle = d.querySelector("#pw-toggle");
    pwToggle.addEventListener("click", function () {
      var isPw = pwInput.type === "password";
      pwInput.type = isPw ? "text" : "password";
      pwToggle.innerHTML = isPw ? SVG.eyeOff : SVG.eye;
    });
    ...
    d.querySelector("#login-form").addEventListener("submit", function (e) {
      e.preventDefault();
      var email = d.querySelector("#email").value.trim();
      var password = d.querySelector("#password").value;
      var errEl = d.querySelector("#auth-error");
      var btn = d.querySelector("#login-btn");
      btn.disabled = true; btn.textContent = "Signing in...";
      setTimeout(function () {
        var err = loginAccount(email, password);
        btn.disabled = false; btn.textContent = "Sign In";
        if (err) errEl.innerHTML = '<div class="alert alert-error auth-error">' + err + '</div>';
  ...
  function renderRegister() {
    ...
    d.innerHTML =
      ...
      '<p class="subtitle">Register as a new lecturer</p>' +
      '<div id="auth-error"></div>' +
      ...
      '<div class="form-group"><label for="email">Email Address</label><input type="email" id="email" placeholder="lecturer@school.edu" required autocomplete="email" /></div>' +
      '<div class="form-group"><label for="password">Password</label>' +
      '<div class="password-field"><input type="password" id="password" placeholder="Min 6 characters" required autocomplete="new-password" />' +
      '<button type="button" class="password-toggle" id="pw-toggle">' + SVG.eye + '</button></div>' +
      ...
      '<div class="form-group"><label for="confirm">Confirm Password</label>' +
      '<div class="password-field"><input type="password" id="confirm" placeholder="Re-enter password" required autocomplete="new-password" />' +
      '<button type="button" class="password-toggle" id="pw-toggle2">' + SVG.eye + '</button></div></div>' +
      '<button type="submit" class="btn btn-primary" id="register-btn">Create Account</button>' +
      '</form>' +
      '<div class="auth-links">' +
      '<div class="auth-divider">or</div>' +
      '<span class="auth-link">Already have an account? <a id="go-login">Sign in</a></span>' +
      '</div>';
    var pwInput = d.querySelector("#password");
    var pwToggle = d.querySelector("#pw-toggle");
    var pwToggle2 = d.querySelector("#pw-toggle2");
    var confirmInput = d.querySelector("#confirm");
    var strengthBars = d.querySelectorAll("#pw-strength .password-strength-bar");
    var strengthLabel = d.querySelector("#pw-strength-label");
    function togglePw(input, btn) {
      var isPw = input.type === "password";
      input.type = isPw ? "text" : "password";
      btn.innerHTML = isPw ? SVG.eyeOff : SVG.eye;
    }
    pwToggle.addEventListener("click", function () { togglePw(pwInput, pwToggle); });
    pwToggle2.addEventListener("click", function () { togglePw(confirmInput, pwToggle2); });
    pwInput.addEventListener("input", function () {
      var pw = pwInput.value;
      if (!pw) { strengthBars.forEach(function (b) { b.className = "password-strength-bar"; }); strengthLabel.textContent = ""; return; }
      var s = passwordStrength(pw);
      strengthBars.forEach(function (b, i) { b.className = "password-strength-bar" + (i < s.bars ? " active " + s.level : ""); });
      strengthLabel.textContent = s.label;
      strengthLabel.style.color = s.level === "weak" ? "var(--danger)" : s.level === "medium" ? "var(--warning)" : "var(--success)";
    });
    ...
    d.querySelector("#register-form").addEventListener("submit", function (e) {
      e.preventDefault();
      var name = d.querySelector("#name").value.trim();
      var email = d.querySelector("#email").value.trim();
      var password = d.querySelector("#password").value;
      var confirm = d.querySelector("#confirm").value;
      var errEl = d.querySelector("#auth-error");
      var btn = d.querySelector("#register-btn");
      if (password !== confirm) { errEl.innerHTML = '<div class="alert alert-error auth-error">Passwords do not match.</div>'; return; }
      btn.disabled = true; btn.textContent = "Creating account...";
      setTimeout(function () {
        var err = registerAccount(name, email, password);
  ...
  function renderForgot() {
    ...
    d.innerHTML =
      ...
      '<div class="password-strength" id="pw-strength"><div class="password-strength-bar"></div><div class="password-strength-bar"></div><div class="password-strength-bar"></div></div>' +
      '<div class="password-strength-label" id="pw-strength-label"></div></div>' +
      '<div class="form-group"><label for="confirm">Confirm New Password</label>' +
      '<div class="password-field"><input type="password" id="confirm" placeholder="Re-enter password" required autocomplete="new-password" />' +
      '<button type="button" class="password-toggle" id="pw-toggle2">' + SVG.eye + '</button></div></div>' +
      '<button type="submit" class="btn btn-primary" id="reset-btn">Reset Password</button>' +
      '</form>' +
      '<div class="auth-links">' +
      '<span class="auth-link">Remembered your password? <a id="go-login">Sign in</a></span>' +
      '</div>';
    var pwInput = d.querySelector("#password");
    var pwToggle = d.querySelector("#pw-toggle");
    var pwToggle2 = d.querySelector("#pw-toggle2");
    ...
    d.querySelector("#forgot-form").addEventListener("submit", function (e) {
      ...
      var confirm = d.querySelector("#confirm").value;
      var errEl = d.querySelector("#auth-error");
      var successEl = d.querySelector("#auth-success-box");
      var btn = d.querySelector("#reset-btn");
      if (password !== confirm) { errEl.innerHTML = '<div class="alert alert-error auth-error">Passwords do not match.</div>'; return; }
      btn.disabled = true; btn.textContent = "Resetting...";
      setTimeout(function () {
        var err = resetPassword(email, password);
        btn.disabled = false; btn.textContent = "Reset Password";
        if (err) { errEl.innerHTML = '<div class="alert alert-error auth-error">' + err + '</div>'; successEl.innerHTML = ""; }
        else {
          errEl.innerHTML = "";
          successEl.innerHTML = '<div class="auth-success">Password reset successfully! You can now sign in with your new password.</div>';
  ...
  function renderLayout() {
    ...
    // Sidebar avatar: show uploaded photo if present, otherwise SVG icon placeholder
  ...
  function attachNavHandlers() {
    document.querySelectorAll(".nav-item").forEach(function (i) {
      i.addEventListener("click", function () {
        state.view = i.getAttribute("data-view");
  ...
  // Student avatar: show uploaded photo if present, otherwise SVG icon placeholder
  function studentAvatarHTML(s, sz) {
    sz = sz || 40;
    var style = 'width:' + sz + 'px;height:' + sz + 'px';
    if (s.photo) return '<div class="student-avatar" style="' + style + '"><img src="' + esc(s.photo) + '"/></div>';
    return '<div class="student-avatar" style="' + style + '">' + avatarPlaceholderHTML() + '</div>';
  }

  function emptyState(img, msg) {
  ...
  function renderDashboard() {
    ...
    var aH = risk.length ? '<div class="alert-banner"><span class="alert-icon">' + SVG.alert + '</span><span><strong>' + risk.length + ' student' + (risk.length > 1 ? "s" : "") + '</strong> below 75% attendance. <a href="#" id="view-atrisk" style="color:var(--danger);font-weight:600;">View list</a></span></div>' : "";
    var snap = tR.length ?
    ...
    var rT = total ?
      '<div class="table-wrap"><table><thead><tr><th>Name</th><th>Matric</th><th>Course</th><th>Level</th></tr></thead><tbody>' +
    ...
    if (gb) gb.addEventListener("click", function () { state.view = "students"; render(); });
    var rl = d.querySelector("#view-atrisk");
    if (rl) rl.addEventListener("click", function (e) { e.preventDefault(); state.view = "atrisk"; render(); });
  ...
  function renderStudents() {
    ...
    var d = document.createElement("div");
    d.innerHTML =
      '<div class="toolbar"><div class="search-wrapper"><span class="search-icon">' + SVG.search + '</span><input type="text" class="search-input" id="student-search" placeholder="Search by name, matric, or course..." /></div><div class="spacer"></div>' +
  ...
  function renderStudentTable(q) {
    ...
    c.innerHTML = '<div class="table-wrap"><table><thead><tr><th>Name</th><th>Matric</th><th>Course</th><th>Level</th><th>Attendance</th><th>Actions</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
  ...
  function exportStudentsCSV() {
    var rows = [["Name", "Matric", "Course", "Level", "Email", "Phone", "Present", "Absent", "Late", "Total", "Rate (%)", "Streak"]];
    state.students.forEach(function (s) { var sm = attendanceSummary(s.id); rows.push([s.name, s.matric, s.course, s.level, s.email || "", s.phone || "", sm.present, sm.absent, sm.late, sm.total, sm.rate, attendanceStreak(s.id)]); });
  ...
  function openStudentModal(id) {
    ...
    // Avatar preview: show uploaded photo or SVG icon placeholder
    ...
    ov.innerHTML =
      ...
      '<div class="form-row"><div class="form-group"><label>Matric Number</label><input type="text" id="f-matric" required value="' + (s ? esc(s.matric) : "") + '"/></div><div class="form-group"><label>Academic Level</label><select id="f-level">' + lv + '</select></div></div>' +
    ...
    ov.querySelector("#sf").addEventListener("submit", function (e) {
      e.preventDefault();
      var d2 = { name: ov.querySelector("#f-name").value, matric: ov.querySelector("#f-matric").value, course: ov.querySelector("#f-course").value, level: ov.querySelector("#f-level").value, email: ov.querySelector("#f-email").value, phone: ov.querySelector("#f-phone").value, photo: photoData };
      var cs = ov.querySelector("#f-class"); if (cs) d2.classId = cs.value || null;
      if (!d2.name.trim() || !d2.matric.trim() || !d2.course.trim()) { ov.querySelector("#me").innerHTML = '<div class="alert alert-error">Name, matric, and course are required.</div>'; return; }
      if (state.editingId) { updateStudent(state.editingId, d2); toast("Student updated."); } else { addStudent(d2); toast("Student added."); }
      close();
      if (state.view === "students") renderStudentTable(document.getElementById("student-search") ? document.getElementById("student-search").value : "");
  ...
  function renderStudentProfile(id) {
    ...
    // Profile avatar: show uploaded photo or SVG icon placeholder
  ...
  function renderAttendance() {
    ...
    function loadDraft(dt) {
      ...
      state.attendanceDraft = {}; state.attendanceNotes = {};
      getRoster().forEach(function (s) { var r = ex.find(function (a) { return a.studentId === s.id; }); state.attendanceDraft[s.id] = r ? r.status : "present"; state.attendanceNotes[s.id] = r ? (r.note || "") : ""; });
    }
    ...
    function renderRoster() {
      ...
      rc.innerHTML = ro.map(function (s) {
        ...
        return '<div class="attendance-row"><div class="student-info">' + studentAvatarHTML(s, 40) + '<div class="student-meta"><p>' + esc(s.name) + '</p><span>' + esc(s.matric) + ' · ' + esc(s.level) + ' level</span></div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.35rem;"><div class="status-toggle">' + sB("present", st, s.id) + sB("absent", st, s.id) + sB("late", st, s.id) + '</div><input type="text" class="note-input" data-note="' + s.id + '" placeholder="Add note..." value="' + esc(state.attendanceNotes[s.id] || "") + '" style="width:200px;"/></div></div>';
      }).join("");
      d.querySelector("#att-count").textContent = ro.length + " students";
      rc.querySelectorAll(".status-btn").forEach(function (b) { b.addEventListener("click", function () { state.attendanceDraft[b.getAttribute("data-student")] = b.getAttribute("data-status"); renderRoster(); }); });
      rc.querySelectorAll("[data-note]").forEach(function (ni) { ni.addEventListener("input", function () { state.attendanceNotes[ni.getAttribute("data-note")] = ni.value; }); });
    }
    ...
    d.querySelector("#save-att").addEventListener("click", function () {
      var ro = getRoster(); if (!ro.length) { toast("No students to record."); return; }
  ...
  function renderAtRisk() {
    ...
    if (!risk.length) { d.innerHTML = '<div class="panel"><div class="panel-body">' + emptyState(IMG.emptyCheck, "No at-risk students. Everyone is above 75% attendance.") + '</div></div>'; return d; }
  ...
  function renderClasses() {
    setTopbar("Classes", "Manage classes and group students");
    var d = document.createElement("div");
  ...
  function openClassModal(id) {
    ...
    ov.innerHTML =
      ...
      '<div class="modal-body"><div id="me"></div>' +
      '<form id="cf">' +
      '<div class="form-group"><label>Class Name</label><input type="text" id="c-name" required value="' + (c ? esc(c.name) : "") + '"/></div>' +
      ...
      '<div class="form-actions"><button type="button" class="btn btn-outline" id="cancel">Cancel</button><button type="submit" class="btn btn-primary" style="width:auto;">' + (c ? "Save Changes" : "Create Class") + '</button></div>' +
      '</form></div></div>';
    document.body.appendChild(ov);
    ...
    ov.querySelector("#cf").addEventListener("submit", function (e) {
      ...
      if (id) { updateClass(id, d2); toast("Class updated."); } else { addClass(d2); toast("Class created."); }
      close(); renderView();
    });
  }
  ...
  function renderReports() {
    ...
    var d = document.createElement("div");
    d.innerHTML =
      '<div class="panel"><div class="panel-header"><h2>Filters</h2></div><div class="panel-body"><div class="toolbar"><select id="rep-student"><option value="">All Students</option></select><label style="font-size:0.82rem;color:var(--text-2);">From:</label><input type="date" id="rep-from"/><label style="font-size:0.82rem;color:var(--text-2);">To:</label><input type="date" id="rep-to"/><button class="btn btn-accent btn-sm" id="rep-run">Generate</button><button class="btn btn-outline btn-sm" id="rep-csv">' + icon("download", "icon-sm") + ' Export CSV</button><button class="btn btn-ghost btn-sm btn-icon" id="rep-print" title="Print">' + SVG.print + '</button></div></div></div>' +
    ...
    function runReport() {
      ...
      var recs = state.attendance.filter(function (a) { if (sid && a.studentId !== sid) return false; if (from && a.date < from) return false; if (to && a.date > to) return false; return true; });
      lR = recs;
      ...
      var p = recs.filter(function (a) { return a.status === "present"; }).length, ab = recs.filter(function (a) { return a.status === "absent"; }).length, l = recs.filter(function (a) { return a.status === "late"; }).length, t = recs.length;
      ...
      var bh = '<div class="panel"><div class="panel-header"><h2>Per-Student Breakdown</h2></div><div class="panel-body flush"><div class="table-wrap"><table><thead><tr><th>Student</th><th>Matric</th><th>Present</th><th>Absent</th><th>Late</th><th>Rate</th></tr></thead><tbody>' + br + '</tbody></table></div></div></div>';
      ...
      var dh = '<div class="panel"><div class="panel-header"><h2>Attendance Log</h2></div><div class="panel-body flush"><div class="table-wrap"><table><thead><tr><th>Date</th><th>Student</th><th>Matric</th><th>Status</th><th>Note</th></tr></thead><tbody>' + dr + '</tbody></table></div></div></div>';
      rc.innerHTML = sh + bh + dh;
      rc.querySelectorAll("[data-profile]").forEach(function (r) { r.addEventListener("click", function () { state.profileId = r.getAttribute("data-profile"); state.view = "students"; render(); }); });
    }
    function exportRep() {
      if (!lR.length) { toast("Generate a report first."); return; }
      var rows = [["Date", "Student", "Matric", "Course", "Level", "Status", "Note"]];
      lR.slice().sort(function (a, b) { return b.date.localeCompare(a.date); }).forEach(function (a) { var s = findStudent(a.studentId); rows.push([a.date, s ? s.name : "Unknown", s ? s.matric : "—", s ? s.course : "—", s ? s.level : "—", a.status, a.note || ""]); });
  ...
  function renderProfile() {
    ...
    d.innerHTML =
      ...
      '<form id="pf"><div class="form-group"><label>Full Name</label><input type="text" id="p-name" required value="' + esc(lec.name || "") + '"/></div><div class="form-group"><label>Email</label><input type="email" id="p-email" required value="' + esc(lec.email || "") + '"/></div><div class="form-group"><label>Phone (optional)</label><input type="text" id="p-phone" value="' + esc(lec.phone || "") + '"/></div><div class="form-group"><label>Current Password (required to save changes)</label>' +
      '<div class="password-field"><input type="password" id="p-password" placeholder="Enter your password to confirm" style="padding-right:2.5rem;"/><button type="button" class="password-toggle" id="p-pw-toggle">' + SVG.eye + '</button></div></div>' +
    ...
    var pPwInput = d.querySelector("#p-password");
    var pPwToggle = d.querySelector("#p-pw-toggle");
    pPwToggle.addEventListener("click", function () {
      var isPw = pPwInput.type === "password";
      pPwInput.type = isPw ? "text" : "password";
      pPwToggle.innerHTML = isPw ? SVG.eyeOff : SVG.eye;
    });
    d.querySelector("#pf").addEventListener("submit", function (e) {
      ...
      if (state.currentAccount && state.currentAccount.passwordHash !== hashPassword(pw)) {
        d.querySelector("#pe").innerHTML = '<div class="alert alert-error">Incorrect password. Changes not saved.</div>';
        return;
      }
      updateLecturer(d2); toast("Profile updated."); render();
    });
    return d;
  }
  ...
  function renderSettings() {
    ...
    d.innerHTML =
      ...
      '<div class="panel"><div class="panel-header"><h2>About</h2></div><div class="panel-body"><p class="text-bold">Student Attendance Management System</p><p class="text-sm text-muted mt-1">Version 4.0 · Built with pure HTML, CSS, and JavaScript. SVG icons. Real-world photos from Pexels. Data stored locally in your browser.</p></div></div>';
    d.querySelector("#toggle-theme").addEventListener("click", function () { toggleTheme(); render(); });
    function bindPwToggle(inputId, toggleId) {
      var inp = d.querySelector("#" + inputId);
      var tog = d.querySelector("#" + toggleId);
      tog.addEventListener("click", function () {
        var isPw = inp.type === "password";
        inp.type = isPw ? "text" : "password";
        tog.innerHTML = isPw ? SVG.eyeOff : SVG.eye;
      });
    }
    bindPwToggle("cpw-current", "cpw-toggle1");
    bindPwToggle("cpw-new", "cpw-toggle2");
    bindPwToggle("cpw-confirm", "cpw-toggle3");
    d.querySelector("#change-pw-form").addEventListener("submit", function (e) {
      e.preventDefault();
      var current = d.querySelector("#cpw-current").value;
      var newPw = d.querySelector("#cpw-new").value;
      var confirmPw = d.querySelector("#cpw-confirm").value;
      var errEl = d.querySelector("#cpw-error");
      if (!state.currentAccount) { errEl.innerHTML = '<div class="alert alert-error">No account session found.</div>'; return; }
      if (state.currentAccount.passwordHash !== hashPassword(current)) { errEl.innerHTML = '<div class="alert alert-error">Current password is incorrect.</div>'; return; }
      if (newPw.length < 6) { errEl.innerHTML = '<div class="alert alert-error">New password must be at least 6 characters.</div>'; return; }
      if (newPw !== confirmPw) { errEl.innerHTML = '<div class="alert alert-error">New passwords do not match.</div>'; return; }
      state.currentAccount.passwordHash = hashPassword(newPw);
      persist();
      errEl.innerHTML = '<div class="auth-success">Password updated successfully.</div>';
